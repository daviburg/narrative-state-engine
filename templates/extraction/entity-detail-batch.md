...
Given a turn of transcript text and SEVERAL entities (each in its own hard-delimited block, with that entity's prior catalog state as context), extract or update EACH entity independently based on what this turn reveals. Each block is delimited by `===== ENTITY BLOCK N START =====` / `===== ENTITY BLOCK N END =====` markers.

CRITICAL — no cross-entity bleed: Process each block independently. Information about one entity must NEVER be copied into another entity's output. Only use the turn text and that block's own prior state for that block's entity. Do not merge, conflate, or confuse entities that share a scene.
...
For EACH entity, produce a JSON object conforming to this V2 structure:
...
MERGE RULES (apply to each block's entity independently):
...
Rules:
- Only include information supported by the provided turn text and the block's existing entry.
- Do NOT invent attributes or details not present in the text.
- Preserve all existing stable_attributes from the block's current entry; add new ones revealed in this turn. Exception: for char-player, only preserve attributes whose keys are in the allowed set listed below — drop any transient action keys that may exist in the current entry.
...
- PERSONAL NAME COREFERENCE: canonicalize to the fullest form ONLY when it is the SAME person. When a character is referred to by only a given name or only a surname, but the catalog (or this turn) establishes a fuller compound name for that SAME person, set "name" to the most complete form and record the shorter form(s) in stable_attributes.aliases. Likewise, if this entity arrived under a bare given/surname and the turn reveals the full name, expand "name" to the full form and keep the bare form as an alias. A shared name token is necessary but NOT sufficient: if the fuller name conflicts on the other component (a different surname with the same given name, or a different given name with the same surname), it is a DIFFERENT person — do NOT rename or alias; leave this entity unchanged. Fresh-introduction context also overrides the merge: if the turn introduces a bare-name character as a new or distinct individual ("for the first time", "a stranger", "steps forward", "appears", "another <name>", or an appositive role that conflicts with the established character), treat it as a different person — do NOT rename or alias into the existing entry. The goal is one canonical entry per person whose aliases list every surface form, so later turns resolve to this id instead of fragmenting.
...
OUTPUT FORMAT:
Return a single JSON object with one key "entities" whose value is an ARRAY containing exactly one entity object per input block, each with its correct "id". Return the entities in the same order as the input blocks. Do NOT wrap entities under any other key, and do NOT omit any block.
Example (two input blocks): {"entities": [{"id": "char-elder", "name": "The elder", "type": "character", "identity": "An elderly authority figure in the tribal community, known for gnarled hands and a sharp gaze.", "current_status": "Speaking with the player at the council fire, assigning a new task.", "status_updated_turn": "turn-019", "stable_attributes": {"role": {"value": "tribal leader", "inference": true, "confidence": 0.8, "source_turn": "turn-019"}}, "volatile_state": {"condition": "alert and engaged", "location": "loc-council-fire", "last_updated_turn": "turn-019"}, "first_seen_turn": "turn-005", "last_updated_turn": "turn-019"}, {"id": "loc-council-fire", "name": "Council Fire", "type": "location", "identity": "The central gathering fire where the tribe holds council.", "current_status": "Lit, with the elder and player present.", "status_updated_turn": "turn-019", "volatile_state": {"condition": "burning low", "last_updated_turn": "turn-019"}, "first_seen_turn": "turn-008", "last_updated_turn": "turn-019"}]}
