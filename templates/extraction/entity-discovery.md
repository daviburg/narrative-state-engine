...
- CROSS-TURN COREFERENCE: Player turns frequently rename or give titles to entities that were just introduced in the preceding DM turn by description alone. When a player turn names or labels an entity (e.g., calls someone "the elder"), compare by role, appearance, location, interaction, and narrative context, not just by exact name match. Only set is_new to false when there is a single, clearly supported match from the immediately preceding DM turn, with role plus unique contextual anchors that distinguish it from any other plausible entity in that turn. If multiple plausible matches exist, do NOT merge based on role/context alone; leave the reference unresolved (skip it if it is purely anaphoric) or keep is_new true with appropriately lower confidence only if the current turn provides a distinct, catalog-worthy mention under the other rules.
