...
- **Coreference examples** — common RPG patterns where the SAME entity appears with different surface forms across turns. In each case, use existing_id to match:
  ...
  - Personal-name components: a character introduced by a compound personal name is the SAME entity when later referred to by ONLY their given name OR ONLY their surname/family name AND the context reads as a CALLBACK to that established character. "Mara Veylin" → "Mara" (given name alone) → "Veylin" (surname alone) = ONE character. The reverse also holds: if "Veylin" was catalogued first and "Mara Veylin" appears later, that is the same entity WHEN context continuity holds — i.e. it reads as a callback to the established "Veylin", NOT a fresh introduction of a distinct person who merely shares that surname (use existing_id `char-veylin`; the entity-detail phase updates the display name to the full form). Do NOT mint a new id for a name component — match the component to the existing full-name entity (or the existing component entity) by shared given/surname token. NEGATIVE example (a shared token is necessary but NOT sufficient): if "Mara Veylin" already exists and a NEW "Joren Veylin" appears, the conflicting given name ("Joren" ≠ "Mara") means these are TWO DIFFERENT people who happen to share a surname — mint a new id, do NOT merge. NEGATIVE example (fresh introduction overrides the merge): if "Mara Veylin" already exists and the text says "Mara, the baker, steps forward for the first time" (or "appears", "a stranger named Mara", "another Mara"), this is a BRAND-NEW person who merely shares the given name — mint a NEW id, do NOT fuse them into the existing Mara. Likewise, a bare "Mara" when TWO different "Mara"s already exist is AMBIGUOUS — do not auto-merge; rely on surrounding context or mint a distinct id.
...
- PERSONAL NAME COREFERENCE: before minting a new `char-*` id for a personal name, check whether the proposed name shares a given-name OR surname token with an existing character. A shared token is NECESSARY BUT NOT SUFFICIENT to merge. Emit an existing_id (is_new=false) ONLY when ALL THREE hold: (a) the proposed name shares a given/surname token with EXACTLY ONE existing character — if two or more existing characters share the token, a bare reference is AMBIGUOUS, so do NOT guess (mint a distinct id or treat as needing context); AND (b) the proposed name introduces NO CONFLICTING second component — a different surname paired with a matching given name, or a different given name paired with a matching surname, means a DIFFERENT person (e.g. existing "Mara Veylin" vs. new "Joren Veylin" share the surname but conflict on the given name → TWO distinct characters → mint a new id); AND (c) the surrounding context supports CONTINUITY with the existing character — the mention reads as a CALLBACK to someone already established, NOT a FRESH INTRODUCTION of a new individual. MINT A NEW id (do NOT merge) when the bare-name person is introduced as new or distinct: explicit first-introduction language ("for the first time", "a stranger", "a newcomer", "steps forward", "appears", "enters", "we meet"), an APPOSITIVE that conflicts with the known character's role/description ("Mara, the baker" when the known Mara is a guard), "another Mara", "a young Veylin", a different stated relative or origin, etc. When all three conditions hold, a person's given name alone, their surname alone, and their full name are ONE entity — never two or three separate ids (e.g. for an existing "Mara Veylin", a later bare "Mara" or bare "Veylin" both resolve to the same existing id). When ANY condition fails, mint a new proposed_id rather than over-merge. When continuity is genuinely AMBIGUOUS, do NOT guess — mint a new id (fragmentation is recoverable in a later pass; a false merge corrupts identity).
- PERSONAL-NAME CALLBACK FORM: when a personal-name mention IS a continuity callback to an already-catalogued character (all three conditions above hold), emit it in the COMPACT known-entity form `{"existing_id": "char-...", "confidence": ...}` — do NOT emit a bare `name` plus `existing_id`. The compact form is expanded to the catalogued full name downstream, which keeps bare-component callbacks from being dropped as compound-term fragments.
