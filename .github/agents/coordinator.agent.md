...
- After each push to a PR branch, request a fresh Copilot review: `gh api repos/{owner}/{repo}/pulls/{pr}/requested_reviewers -X POST -f "reviewers[]=copilot-pull-request-reviewer[bot]"`. Wait ~15 minutes (use the `wait` tool in 2-minute increments), then check for new inline comments. Include any new comments in the squad loop before declaring readiness.
