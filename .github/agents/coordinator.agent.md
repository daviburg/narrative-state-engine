...
- Require one connectivity attempt per dispatch or resumption, with an explicit timeout. If the named target or required external dependency remains unavailable, mark the existing work item `blocked-external` and record non-secret failure evidence there; do not silently reroute it or create another task with the same target, operation, and SHA.
