...
- **Security-sensitive exception**: Changes involving root access, service definitions or configuration, credential handling, bootstrap, or deployment require a human security reviewer to inspect the staged diff before push. The docs-only shortcut and Copilot-only waiver do not apply. If two review rounds each expose a previously unseen security-blocker class, pause incremental pushes and require one consolidated invariant and threat-model review before further changes.
- ALWAYS check for automated PR review comments (Copilot, CodeQL) after PR creation and include them in the squad loop.
- BEFORE reporting squad consensus to the human, verify PR readiness in order: (1) CI is green, (2) every actionable review root has an exact-form @developer reply and @tester has verified the claim, (3) @tester has classified non-actionable and acknowledgement roots without requiring developer replies and all review threads are resolved, (4) @reviewer gives final GitHub approval, and (5) the PR branch is rebased on latest main with no merge conflicts. A root is actionable when it requests or implies a code, test, documentation, or readiness change; default ambiguous roots to actionable. Parse every automated finding surface: inline review comments, suppressed or body-only review findings, check annotations, and issue-style PR comments. Verify each finding was fixed or has an explicit dismissal/no-change rationale. If behind, dispatch @developer to rebase and repeat CI and tester verification before declaring ready.
- Maintain an exact-head review ledger for every pushed SHA, including its base/main SHA, CI result, automated and human review results, finding dispositions, thread state, and observation times. Before any readiness claim, capture a final snapshot for the current exact head with those fields plus mergeability. A deliberately superseded head may skip a wasteful exact-head review only when the ledger records the old SHA, replacement SHA, reason, owner, and timestamp; no readiness claim is allowed until the final exact head clears every gate.
...
- After each push that is not deliberately superseded under the ledger rule above, dispatch @developer to request a fresh Copilot review via `gh api repos/{owner}/{repo}/pulls/{pr}/requested_reviewers -X POST -f "reviewers[]=copilot-pull-request-reviewer[bot]"`. Then schedule a follow-up task via the orchestrator with `not_before` set to 15 minutes from now to check every automated finding surface (see PR Fix Task Pattern). Include any new findings in the squad loop before declaring readiness.
...
- Track `implementation-complete`, `deploy-ready`, `deployed`, and `production-validated` independently, advancing each only from direct evidence. A PR merge, task submission, or successful dispatch never proves `deployed` or `production-validated`.
- In the evidence ledger, distinguish `task_transport_succeeded`, `implementation_verified`, `workflow_complete`, `pr_ready`, `merged`, `bootstrap_approved`, `deployed`, and `live_validated` (the evidence-ledger form of the `production-validated` milestone). Advance each state only from its own evidence; orchestrator task success proves only `task_transport_succeeded`, never domain success or any later state.
- Before dispatching host-changing work, record the exact target and owner, pinned commit SHA or artifact digest, prerequisites, a preflight timeout, rollback procedure, and required post-deploy checks with their evidence destination.
- Deployment approval must bind the merged commit SHA and tree, artifact digest, target, operator, live-validation gate, and rollback procedure. Green code review or CI alone never authorizes deployment.
- Require one connectivity attempt per dispatch or resumption, with an explicit timeout. If the named target or required external dependency remains unavailable after the timed attempt, mark the existing work item `blocked-external`. This state means the named target or required external dependency remains unavailable after that attempt; record non-secret failure evidence in the work item, and do not silently reroute it or create another task with the same target, operation, and pinned identifier (commit SHA or artifact digest).
