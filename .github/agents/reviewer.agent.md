...
10. **Review thread closure**: Enumerate all paginated review comments and identify every root comment, including automated-review roots. Require a substantive `**[@developer]**` thread reply that names the fixing commit SHA and change or gives a no-change rationale. Independently verify zero unresolved threads and zero root comments without a developer reply; either condition blocks merge. Check annotations and issue-style comments separately.
11. **Parser extensions**: Verify the change stays within the repository-owned extension layer and delegates baseline grammar, tokenization, and error semantics to the host parser. Require adversarial grammar coverage and differential tests for syntax shared with the host parser.
12. **Renderer/browser changes**: Require tests through the real production page and shipped assets. Verify stage-failure fallback, history/live parity, accessible state exposure, and responsive containment at supported viewport bounds.
13. **Minimum runtime**: Compute the effective floor from locked direct and transitive dependency engine constraints. Require a strict install and relevant tests at the exact CI floor, with manifest, lockfile, CI matrix, and user-facing documentation synchronized.
