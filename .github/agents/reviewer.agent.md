...
10. **Review thread closure**: Independently run the paginated REST root/reply audit in @developer's Review feedback step and the GraphQL unresolved-thread query in @tester's Comment Verification Protocol; do not rely on a relayed summary. Every actionable inline root must have one of the exact `**[@developer]** Fixed in <sha>: <description>`, `**[@developer]** Tracked as follow-up in #NNN: <description>`, or `**[@developer]** No change: <rationale>` replies. Acknowledgements and non-actionable informational roots do not require a developer reply. Any unresolved thread or actionable root without the required reply blocks merge, but @tester retains ownership of verifying claims and resolving threads. Check automated findings delivered as annotations or issue-style comments separately: they are excluded from threaded reply requirements but must be fixed or have an explicit dismissal/no-change rationale.
