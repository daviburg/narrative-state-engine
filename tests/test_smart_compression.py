...
class TestConstantValues:
    ...
    def test_entity_budget_fraction_restored_to_25_percent(self):
        # Entity-retention rework (#460): restored from the 0.15 that starved
        # discovery (peripheral entities never proposed) back to the 0.25
        # baseline. The speed/token gain comes from per-entity detail trims,
        # not from squeezing the discovery context budget.
        assert _DEFAULT_ENTITY_BUDGET_FRACTION == 0.25

    def test_staleness_threshold_restored_to_50(self):
        # Entity-retention rework (#460): restored from the 0.15-era 30 back to
        # the 50 baseline so recently-seen peripheral entities are not dropped.
        assert _DEFAULT_STALENESS_THRESHOLD == 50

    def test_discovery_staleness_at_least_default(self):
        # Discovery must never see a tighter staleness window than the general
        # entity context, or late-game turns starve of catalog awareness (#460).
