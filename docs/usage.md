...
| `context_optimizations.checkpoint_compaction` | Boolean. Enables the Phase A0 periodic checkpoint-compaction digest body (epic #477, design `docs/design-context-architecture-bounded.md` §9.1; Spike F10). **Default: `false`** (byte-identical to main — flipping the default to `true` requires a separate A/B-gated follow-up PR validated against the new flag-ON 344t reference baseline). When `true`: on a fixed `compaction_interval_k`-turn cadence the accumulated prior state is compacted into a deterministic, **extractive** checkpoint snapshot plus an **append-only recent-delta buffer** (no extra LLM call, byte-stable at temperature 0), bounding staleness to ≤ K turns. (entity_detail) `_format_prior_entity_context` uses `_build_checkpoint_compacted_volatile()` in place of the sliding `_DIGEST_WINDOW=50` digest — volatile entries at or before the most recent checkpoint boundary (`floor(t/K)*K`) fold into one snapshot summary, entries after it are the verbatim delta. (discovery) `format_known_entities_bounded` renders non-recent, non-priority pre-checkpoint snapshot entities in compact id-only form while leaving the full-detail recent/priority set identical to OFF (coreference floor preserved, never larger than OFF). Scope is A0 only: no eviction, cold-storage rollup (A1b), or A2 retrieval. |
| `context_optimizations.compaction_interval_k` | Integer. Checkpoint cadence K (turns between snapshots) for `checkpoint_compaction`. **Default: `25`**. This is a **distinct** key from the top-level disk-persistence `checkpoint_interval` (#220/#212 crash recovery): they are different concerns and are **not** overloaded even though both default to 25. Invalid or missing values fall back to 25. |
