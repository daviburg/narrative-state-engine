...
| `context_optimizations.pc_rel_permanent_types` | Array of relationship-type strings treated as permanent bonds (never dropped by the volatile tail cap). **Default: `["kinship","adversarial","mentorship","political","partnership"]`** (all members of the relationship `type` enum in `schemas/entity.schema.json`). These are structural schema-type enum values (not domain content words — see Rule 9). |
