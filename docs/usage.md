...
| `context_optimizations.relationship_type_tiering` | Boolean. Enables L1+L4 type-tiered PC relationship cap (epic #477). **Default: `false`** (same-commit flag-OFF run serves as A/B control). When `true`: (L1) `_format_prior_entity_context` applies `_apply_pc_rel_type_tier()` to the PC relationship list — permanent-bond types (see `pc_rel_permanent_types`) are kept uncapped, any relationship whose target is mentioned this turn is force-kept, and the remaining volatile tail is capped at `pc_rel_volatile_tail_cap`. (L4) `_collect_existing_relationships` promotes permanent-bond types to at least tier 2 (never omit) so the volatile tail is trimmed first under budget pressure. Closes the #385 PC gap. |
| `context_optimizations.pc_rel_permanent_types` | Array of relationship-type strings treated as permanent bonds (never dropped by the volatile tail cap). **Default: `["kinship","adversarial","mentorship","political","partnership","captor","debt"]`**. These are structural schema-type enum values (not domain content words — see Rule 9). |
