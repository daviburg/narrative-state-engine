...
`--noise-floor` is then in tok/turn units. Because the per-call default
(`5.0` tok/call) would be an unrealistically tiny floor for tok/turn totals
(and could yield a misleading `SEPARABLE` verdict), `--per-turn` has **no
default noise floor**: you must pass an explicit `--noise-floor` re-measured
from an A==A per-turn control, or the tool exits with an error.
...
| `relationship_max_tokens` | Max output tokens for the **relationship-mapper phase** only. Defaults to `max_tokens` if omitted; the shipped config sets `8192`. This is a ceiling, not a target: at temperature 0 / `top_k` 1 greedy decoding stops at the natural end of the JSON, so turns whose relationship output fits under the default are byte-identical. The higher ceiling avoids a wasteful truncation + full restart on rare entity-dense council turns. The truncation retry/repair safety net is retained for a runaway beyond the ceiling. |
...
| `context_optimizations` | Optional object. Houses feature-flagged context budget levers. All sub-keys default to their documented default when the block is absent. |
...
| `context_optimizations.batch_entity_detail` | Optional object (L2, epic #477, #491). Batches `entity_detail` LLM calls so the system template + turn text are sent once per batch instead of once per entity. Sub-keys: `enabled` (boolean, **default `false`** — flag-OFF is byte-identical to the per-entity baseline / A/B control; only a real JSON `true` enables it); `batch_size` (integer, **default `4`**; configured values `< 2` are ignored and fall back to the default — batching a single entity saves nothing — so the effective size is always ≥2; the lower-salience tail is batched in groups of this size; balances token saving against cross-entity attribute-bleed risk and per-call output growth, Rule 10); `high_confidence_threshold` (number, **default `0.8`** — entities with `confidence` at/above this stay solo). The player character, brand-new entities, and high-confidence entities are always kept solo; parse failure, or any missing/invalid/duplicate/misattributed entity, falls back to a per-entity solo call (a returned object whose name/type does not match its requested block is rejected) so no entity is dropped or silently misattributed; fallback solo prompts are counted in the per-turn token metric. Score with `tools/ab_paired_score.py --per-turn` (batching changes the call count by design) and gate on the #488 paired A/B (≥3 runs/arm) before flipping the default. |
...
- `discovery_filtered` — array of entities rejected during filtering, each with `name`, `id`, and `reason`. Possible `reason` values:
  - `below_confidence_threshold` — the model's proposed `confidence` was below the configured minimum (#250)
  - `concept_prefix` — the entity's resolved id has a `concept-` prefix, which is never a valid catalog type (#124)
  - `misclassified_character` — the entity was proposed as a `character` but its name matches a non-sentient naming pattern (#303)
  - `misclassified_location` — the entity was proposed as a `location` but its name matches a non-spatial naming pattern (#303)
  - `misclassified_item` — the entity was proposed as an `item` but its name matches an abstract-concept naming pattern (#339)
  - `cross_catalog_type_conflict` — the entity's name already exists in the catalog under a different `type` (includes a `conflicting_id` field with the existing entity's id) (#303)
  - `compound_term_fragment` — the entity's single-word name was found in the runtime compound-term index built from multi-word entity names in the catalog and current turn, AND the candidate's `type` matches the type of the multi-word entity that contributed the word (includes a `compound_name` field) (e.g., "quiet" rejected because location "Quiet Weave" exists; but item "ledger" is NOT rejected merely because a faction "Red Ledger Syndicate" exists — different types cannot fragment one another) (#398, #539)
  - `unresolvable_existing_id` — the entity referenced an `existing_id` that does not resolve to any real catalog entry, and could not be safely rescued as a genuinely new entity (#524)
