...
| `dedup_audit_interval` | Run LLM-assisted dedup audit every N turns during extraction. Default: `50`. Set to `0` to disable. The periodic audit catches entity evolution (e.g., same location renamed or upgraded over time) that string-similarity dedup cannot detect. Uses an enhanced scoring prompt with narrative evolution awareness. |
