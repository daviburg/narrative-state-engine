...
The three compression surfaces (`format_known_entities_bounded`, `_collect_existing_relationships`, `_trim_entry_for_scene`) expose an opt-in `return_stats` path that yields `(text, stats)` instead of `text`; the stats carry the raw/compressed token estimates plus section drop counters. PR-2 consumes these; PR-1 ships them dormant and unit-tested.
