...
- **Adaptive `max_tokens`**: when the catalog has more than 20 non-trivial entities (those with substantial volatile state or stable attributes), entity detail extraction uses `max_tokens=8192` instead of the default 4096 to prevent `detail_error: truncated` on high-entity-count turns.
