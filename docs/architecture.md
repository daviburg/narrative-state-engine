...
- **Adaptive, pressure-gated compression** (#460, epic #464; default off): An opt-in layer that activates the #465 compression instrumentation by trimming context **only when a phase is under real budget pressure**, while protecting entity discovery from over-trimming (the #393 starvation regression). Controlled by `context_optimizations.adaptive_compression` in `config/llm.json`; when the block is absent or `enabled: false`, `adaptive_compression_config()` returns `None` and every call site short-circuits, so behavior is **byte-for-byte identical** to the always-on baseline and the instrumentation reports exact pass-through (`raw == compressed`, `compression_ratio == 0.0`). When enabled, the activation flow is:
  1. **Pressure gate** (`pressure_gate_fraction`, default **0.45**) — a phase's context is compressed only when its assembled (uncompressed) token estimate exceeds this fraction of the phase budget. Below the gate the full uncompressed context is returned, so early/low-fill turns pass through untouched. *Justification: compressing low-pressure turns starved discovery in #393. Recalibrate on model change* (the `_estimate_tokens` char/token ratio shifts the effective fill).
