...
- **Periodic entity refresh** (#161): Every `entity_refresh_interval` turns (default 50), the pipeline identifies entities whose `last_updated_turn` has fallen behind by more than the interval. Up to `entity_refresh_batch_size` (default 5) of the most stale entities are re-extracted using recent transcript context where they are mentioned. This prevents entities from going permanently stale in long sessions (e.g., an entity last updated at turn-061 in a 345-turn session). The refresh uses the same entity-detail LLM template and merges results via `merge_entity()`.
...
- **Non-standard key coercion** (#170): `_coerce_entity_fields()` remaps common non-standard top-level keys returned by smaller LLMs into their correct V2 schema locations before validation. For example, `equipment`/`inventory` → `volatile_state.equipment`, `location` → `volatile_state.location`, `abilities` → `stable_attributes.abilities`, `relations`/`character_relations` → `relationships`. Known noise keys (`image_url`, `future_plans`, `actions`, etc.) are discarded. This transforms most schema validation failures from creative LLM outputs into successful merges.
