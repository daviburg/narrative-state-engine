...
- **Body part and abstract concept filter** (#338): The post-discovery filters reject non-entity names via two separate paths. `_is_misclassified_location()` rejects body parts (shoulders, lips, body, etc.) and abstract/surface concepts (edge, ground, reaction, pattern) from being classified as locations; possessive body-part phrases ("his lips", "her shoulders") and multi-word names whose head noun is blocklisted (e.g. "the river's reaction") are also caught. `_is_misclassified_character()` rejects body parts, generic group nouns (men, figures, people), and number-as-name words (two, three, etc.) from being classified as characters. Additionally, `_create_orphan_stubs()` and `_post_batch_orphan_sweep()` check `_STUB_REJECT_STEMS` (combining body parts and non-entity abstract/group terms) so that filtered IDs cannot re-enter the catalogs via event stubs.
