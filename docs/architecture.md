...
- **Dedup audit** (`tools/dedup_audit.py`): Post-extraction tool that identifies duplicate entities via name similarity heuristics (Levenshtein distance ≤3, substring match, same-turn co-occurrence), scores candidate pairs with an LLM call (confidence 0–1), and either auto-merges high-confidence duplicates (≥0.9) via coreference hints or flags medium-confidence pairs (0.6–0.9) for human review in `dedup-review.json`. Pairs where both entities have ≥3 relationships to distinct targets are excluded from candidacy to avoid false positives.
