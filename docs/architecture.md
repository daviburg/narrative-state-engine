...
- **Robust split + fallback**: `_parse_batch_detail_response()` splits the batched response back to per-entity results by `id` (accepting the `{"entities": [...]}` envelope, a bare list, or a single-entity envelope). `_extract_batched_entity_detail()` falls back to a **per-entity solo call** for any entity missing from, or invalid in, the batched response, and for a whole-batch parse/LLM failure — so no entity is ever dropped or merged away. Both the sequential and `parallel_workers > 1` paths consume the batched results uniformly.
- **Non-blind measurement (the A0 lesson)**: the recorded `entity_detail` metric must reflect the prompt L2 actually changes. The per-turn instrumentation records the **actual batched prompt** (both the compacted-sent `input_tokens` and the #484 uncompacted `raw_input_tokens`) once per group, so the per-turn `raw_input_tokens` (a sum across calls) drops by ~`(K-1) × (template + turn_text)` for a batched turn of `K` entities. Because L2 *reduces the entity_detail call count by design*, the default per-call `matched-call-COUNT` scorer would drop every batched turn; `tools/ab_paired_score.py --per-turn` scores the **per-turn TOTAL** `raw_input_tokens` over the matched-TURN population instead, which is the fair, non-blind comparison for L2 (`--noise-floor` is then in tok/turn units). The #488 paired-A/B gate still applies: validate with ≥3 runs/arm before flipping the default.
