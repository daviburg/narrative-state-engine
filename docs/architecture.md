...
- **Body part and abstract concept filter** (#338): `_is_misclassified_location()` and `_is_misclassified_character()` reject body parts (shoulders, lips, body, etc.), abstract narrative concepts (edge, ground, reaction, pattern), and generic group nouns (men, figures, two) from entity discovery. Body part names in possessive phrases ("his lips", "her shoulders") are also caught. Multi-word location names whose head noun (last word) is a blocklisted term (e.g. "the river's reaction") are rejected via head-noun matching.
