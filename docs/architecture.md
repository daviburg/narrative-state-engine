...
- **Adaptive, pressure-gated compression** (#460, epic #464; default off): An opt-in layer that activates the #465 compression instrumentation by trimming context **only when a phase is under real budget pressure**, while protecting entity discovery from over-trimming (the #393 starvation regression). Controlled by `context_optimizations.adaptive_compression` in `config/llm.json`; when the block is absent or `enabled: false`, `adaptive_compression_config()` returns `None` and every call site short-circuits, so behavior is **byte-for-byte identical** to the always-on baseline and the instrumentation reports exact pass-through (`raw == compressed`, `compression_ratio == 1.0`; the stderr line reads `ratio=1.00 (INACTIVE)`). The ratio is `compressed / raw`, so a faithful no-op is `1.0`, never `0.0`. When enabled, the activation flow is:
  1. **Pressure gate** (`pressure_gate_fraction`, default **0.45**) — a phase's context is compressed only when its assembled (uncompressed) token estimate exceeds this fraction of the phase budget, and the degrade/omit passes trim back **toward that same threshold** (never below the discovery floor). Below the gate the full uncompressed context is returned, so early/low-fill turns pass through untouched. The fraction therefore controls both *when* and *how hard* a phase compresses: `pressure_gate_fraction == 1.0` reproduces the original "compress only on hard budget overflow" behavior, while `0.45` compresses once the phase is ~45% full. *Justification: compressing low-pressure turns starved discovery in #393. Recalibrate on model change* (the `_estimate_tokens` char/token ratio shifts the effective fill).
  2. **Discovery floor** (`discovery_floor_fraction`, default **0.25**) — in `format_known_entities_bounded()` the retained discovery context is never trimmed below this fraction of the discovery budget, even under heavy pressure; the degrade and omit passes stop at the floor. Only the turn-total cap may override it, and only after trimming other phases first. *Justification: protects against the #393 discovery-starvation entity loss. Recalibrate on model change.*
  3. **Centrality backstop** (`centrality_min_degree`, default **2**; optional `centrality_exempt_top_n`, default `null`) — a purely structural per-entity centrality score (`compute_entity_centrality()`: outbound `relationships[].target_id` degree + inbound references + mention frequency from `mention_count`/`source_turns`; **no domain word list — Rule 9**) exempts high-centrality entities from the normal degrade/omit passes (Passes 1–3). They are degraded or omitted only as a **last resort** (Pass 4), tail-first, when so many entities are exempt that the prior passes could not free enough room and the known-entities section would otherwise exit *above its own hard token budget* — and never below the discovery floor. The turn-total cap can likewise reformat exempt content only after trimming lower-priority phases first. In short, exemption protects central entities from routine trimming but does not make them strictly inviolable: a binding hard budget (per-section or turn-total) can still trim them, bounded by the discovery floor. *Justification: structurally central entities are the ones whose loss most degrades extraction. Recalibrate on model change.*
  4. **Turn-total budget** (`turn_total_budget_fraction`, default **0.85**) — `coordinate_turn_total()` is a turn-level aggregation point that **computes and reports** a best-effort per-turn allocation: at the end of `extract_and_merge()` it sums the assembled context across phases and, when that sum exceeds this fraction of `context_length`, allocates the overflow away from the lowest-priority phases first, never below each phase's floor (so the discovery floor and centrality-exempt content are preserved). The result is surfaced as `turn_total_allocated`/`turn_total_cap` in the per-turn compression metrics. **It does not currently feed back into prompt construction** — it is computed *after* all phase prompts are already assembled and does not trim any already-built prompt, so the cap is a reporting/diagnostic metric rather than a runtime-enforced limit (wiring it into prompt construction is a downstream task). *Justification: leaves headroom for the response while bounding total prompt growth. Recalibrate on model change* (`context_length` and the response budget both shift). **#465 instrumentation wiring:** when adaptive trimming runs, the discovery phase reports the real raw-vs-compressed delta through `_record_prompt_tokens(raw_tokens=…, section_stats=…)`, so `raw_input_tokens`/`compressed_input_tokens`/`compression_ratio` and the `[COMPRESSION]`/`[RETENTION]` stderr lines carry true numbers; with the flag off the delta is 0 and the lines report `ratio=1.00 (INACTIVE)`.
