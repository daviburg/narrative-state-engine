...
- **Logged metrics** (per turn, under `relevance_detail_shadow`): `actual_detailed_count`/`_ids` (the post-cap set actually detailed), `cap_dropped_count`/`_ids` (what the cap-6 dropped), `relevance_would_select_count`/`_ids` (what relevance would pick — bounded by scene size, ~constant, vs the cap-6 truncation), `cap_dropped_but_would_include_count`/`_ids` (the silently-dropped updates relevance recovers), and the **critical safety metric** `referenced_but_would_drop_count`/`_ids` — entities **referenced/mentioned in the turn that relevance would drop**. A referenced entity being dropped is the coreference-safety failure mode, so this must be ~0 for the signal to be sound; because mentioned entities are tier-1 in the #233 signal, the shadow demonstrates it is 0 by construction.
