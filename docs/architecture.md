...
- **Compound-term fragment rejection**: The entity discovery template (`templates/extraction/entity-discovery.md`) includes an ENTITY NAME VALIDATION section that instructs the LLM to reject single-word fragments of compound terms (e.g., do not create `char-quiet` from "Quiet Weave") and treat single common English words as non-character names unless capitalized and directly addressed.
...
- **Same-turn dedup gate** (#337): After discovery acceptance filters, `_same_turn_dedup_gate()` detects synonym explosion — when >3 new entities of the same type share a `source_turn`. Only the entity with the longest (most specific) name survives. This prevents a single DM description using multiple synonyms (e.g. "safe haven", "hidden refuge", "defensible sanctuary") from creating 14 separate location entities (as observed in run16 turn-169). The gate only affects new entity proposals; existing entity references pass through unchanged. The extraction log records a `synonym_explosion_warning` when the threshold is exceeded.
