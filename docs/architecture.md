...
- **What it captures**: when ON, the `LLMClient` (`tools/llm_client.py`) tees one JSONL record per call to `<framework-dir>/raw-io-capture.jsonl` — kept **separate** from `extraction-log.jsonl` because the raw text is large. Each record is `{timestamp, turn, phase, entity_id, raw_prompt:{system,user}, raw_completion, input_tokens, output_tokens, input_tokens_estimated, output_tokens_estimated}`. `phase` ∈ {`entity_detail`, `entity_detail_batch`, `discovery`, `relationship_mapper`, `event`} (`generate_text` callers may pass others, e.g. `synthesis`); `entity_id` is attached for the per-entity `entity_detail` calls (including the PC call, tagged `char-player`), so the **per-entity PC-call-isolated** input/output tokens are recoverable.
- **Output (decode) tokens — the never-before-logged 87%-wall-clock driver**: `output_tokens` is taken from the backend's real `usage.completion_tokens` when the response exposes it (`output_tokens_estimated: false`); otherwise it falls back to a char-heuristic estimate of the raw completion (`_estimate_tokens`) flagged `output_tokens_estimated: true`. `input_tokens` follows the same rule via `usage.prompt_tokens`. Per-call input tokens already existed (#484) as a pre-compaction measure; this records the **actual sent** call's tokens per entity.
- **Where it's wired**: the capture point is the `LLMClient` boundary (`extract_json` / `generate_text`), which already holds the raw prompt + raw response + usage for every call. The phase callers in `tools/semantic_extraction.py` pass a small `capture={"turn","phase","entity_id"}` tag through; the client writes the record only when capture is enabled **and** a tag is supplied. The writer is **best-effort** (any serialisation/IO error is swallowed) so capture can never interrupt extraction.
