...
- **Context-aware entity selection** (#233): When the current turn's text is available, `format_known_entities_bounded()` uses a multi-tier selection strategy instead of recency-only ordering. Priority tiers: (1) **Mentioned** — entities whose name or alias appears in the turn text (case-insensitive substring match, names shorter than 3 characters skipped); (2) **Co-located** — entities sharing a `volatile_state.location` with a mentioned entity, or entities at a mentioned location; (3) **One-hop relationships** — `relationships[].target_id` targets of mentioned entities; (4) **Recency backfill** — remaining entities sorted by `last_updated_turn` descending. Within each tier, entities are sorted by recency. Mentioned, co-located, and one-hop entities receive full detail regardless of recency; backfill entities use the same recency-window logic as before. Budget enforcement is unchanged — context-aware selection decides *what* fills the budget, not *how much*. When `turn_text` is not provided, the function falls back to the pre-#233 recency-only behavior.
