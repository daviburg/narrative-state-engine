...
- **Structured extraction log** (#217): Every call to `extract_and_merge()` returns a per-turn log record capturing the outcome of each extraction phase (discovery, detail, PC, relationships, events), any error messages, counts of new entities/events, and elapsed time. In batch, segmented, and single-turn modes the record is appended to `<framework-dir>/extraction-log.jsonl` (one JSON object per line). The file is append-only and written after each turn, so it survives interruptions. It immediately answers which turns failed, what error each phase produced, and when failures started. Skipped in `dry_run` mode.
