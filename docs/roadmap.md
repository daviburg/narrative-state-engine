...
- **Adaptive, pressure-gated context compression** (#460, epic #464; **PR-2 implemented, A/B pending**): Activates the #465 compression instrumentation with an opt-in (`context_optimizations.adaptive_compression`, default off) layer that compresses a phase only when it is under real budget pressure (`pressure_gate_fraction` 0.45), never trims discovery below a floor (`discovery_floor_fraction` 0.25, anti-#393-starvation), exempts structurally central entities from degrade/omit passes (`centrality_min_degree`, purely structural — Rule 9), and computes and reports a best-effort per-turn cap/allocation (`turn_total_budget_fraction` 0.85) at finalize (a reporting metric, not yet enforced back into prompt construction). With the flag off, behavior is byte-for-byte identical to the baseline. A tolerance-based A/B per `docs/ab-test-standard.md` is required before enabling and is tracked as a downstream task (blocked on #467 merge + an A-vs-A noise-floor baseline).
