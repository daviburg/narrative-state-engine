...
- **Compound-term fragment rejection** (#398): `_build_compound_word_index()` builds a runtime index of every word in every multi-word entity name (normalized to `[a-z]+`), recording the contributing entity's `type` alongside the compound name; `_is_compound_term_fragment()` rejects single-word entities whose name appears in the index **only when the candidate's own `type` matches the indexed entity's `type`** (#539 — a candidate/index entry with an unknown type falls back to the original type-agnostic rejection). This fixed a cross-type false positive where an item named "ledger" was wrongly rejected as a fragment of an unrelated faction "Red Ledger Syndicate". The prompt template (`entity-discovery.md`) reinforces this with an ENTITY NAME VALIDATION section. The index is built from catalog data at runtime — no hardcoded word lists. A/B validation was completed as part of the PR #399 templates process.
...
- **Type-tiered PC relationship cap L1+L4** (epic #477): Closes the #385 gap — the PC entity-detail path and the relationship-mapper budget were previously uncapped for PC relationships (109 active at turn 344, 0 ever resolved). L1 adds `_apply_pc_rel_type_tier()` to `_format_prior_entity_context` for the PC path: permanent-bond types (kinship, adversarial, mentorship, political, partnership) kept uncapped; volatile tail capped at `pc_rel_volatile_tail_cap` (default 10) after mention-force-keep; 109-rel PC web shrinks to ~30–40. L4 adds the same type-tiering to `_collect_existing_relationships` so permanent bonds survive the token-budget degradation step. Gated under `context_optimizations.relationship_type_tiering` in `config/llm.json` (shipped default OFF in #480). A/B validation (344t, same-commit flag-OFF control, port-8000 lane, P1) is a separate follow-up task. **Update:** the 150t A/B measured the flag-ON arm as merge-ready at quality parity (per-call PC-context token slope roughly halved), so the default was flipped from OFF to ON; the new 344t full-context baseline extraction runs on this default-ON state.
- **Relevance-detail shadow S0** (epic #477, lever (b)/A2a, #494): The first slice of relevance-scoped `entity_detail` selection, shipped as **measurement-only** instrumentation. Each turn, `_compute_relevance_detail_shadow()` logs which entities the existing relevance signal (`_select_context_aware_entities`, #233) WOULD pick for detail versus the current cap-6 truncation (`_MAX_DETAIL_ENTITIES_PER_TURN`), including the cap-dropped-but-relevant count and the critical safety metric `referenced_but_would_drop_count` (entities mentioned this turn that relevance would drop — must be ~0 for the signal to be sound). Gated under `context_optimizations.relevance_shadow_logging` in `config/llm.json` (**default OFF**); flag-ON and flag-OFF produce byte-identical extraction output (catalogs, events, detail-call selection), only the extra `relevance_detail_shadow` log stream differs (proven by `tests/test_relevance_detail_shadow.py`). This validates the relevance signal is sound before the behaviour-changing S1 (A2a gate) slice. No GPU A/B is required to merge; the shadow rides along an existing extraction run for measurement.
- **Relevance-scoped entity detail S1a — recover-only** (epic #477, lever (b)/A2a, #498; builds on S0 #494): The first **behaviour-changing** slice. When ON (`context_optimizations.relevance_scoped_detail.enabled = true`, **default OFF**, `mode = "recover_only"`), `_select_relevance_scoped_detail_recover()` makes the per-turn detail set the cap-6 set **UNION** the relevance recoveries — the cap-dropped candidates the #233 signal marks relevant — so the ~245 real updates the confidence-sorted cap-6 silently drops are recovered. Recover-only is a strict **superset** of the cap-6 set: it ADDS recoveries and drops nothing currently detailed (dropping off-scene filler is the separate, not-yet-implemented S1b `mode = "drop_filler"`). `_apply_relevance_ceiling()` bounds the set to `max_detail_entities` (the **ceiling**, default 10 = S0 `would_select` p95), trimming the lowest-relevance recovery tail on bind while force-keeping the floor (PC/new/mentioned, which may exceed the ceiling) and the cap-6 set; the ceiling is clamped `>= _MAX_DETAIL_ENTITIES_PER_TURN = 6`. Flag-OFF is **byte-identical** to the cap-6 baseline (the A/B control), proven by `tests/test_relevance_scoped_detail.py`. Gated on a **quality** update-retention A/B vs a cap-6 control — **not** the 60h token A/B (S1's token delta is ~flat by design); the total-input-token slope remains lever (c)'s (S2/S3) deliverable.
...
- **Bounded per-turn context architecture (A1+A2)** (epic #477): Two-part architecture targeting **bounded per-turn extraction tokens** (where the token-stabilization levers L1/L4/L2 only halve the slope) — **A1** is a bounded, always-in-context identity/alias anchor digest with telemetry-gated intelligent forgetting (reversible promote-on-mention), and **A2** is retrieval-scoped full-fidelity injection of the turn-relevant subset (reusing the #233 relevance selection). Key constraints: **bounded per-turn tokens** plus a **zero-new-duplicate coreference gate**, validated by a long-run (>=150t, ideally the full 344t) A/B; ship L1/L4/L2 first, then A1 (the coreference safety floor), then A2. Full spec, phasing, open design forks, and the folded-in F3 (tokenizer-true identity-line cost, MEASURED 17/19/25 tok/entity) and F10 (checkpoint-compaction adopted as the first build increment) spike results are in `docs/design-context-architecture-bounded.md` (builds on the per-lever decomposition in `docs/design-token-stabilization.md`, PR #478).
