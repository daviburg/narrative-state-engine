...
- **Type-tiered PC relationship cap L1+L4** (epic #477): Closes the #385 gap — the PC entity-detail path and the relationship-mapper budget were previously uncapped for PC relationships (109 active at turn 344, 0 ever resolved). L1 adds `_apply_pc_rel_type_tier()` to `_format_prior_entity_context` for the PC path: permanent-bond types (kinship, adversarial, mentorship, political, partnership, captor, debt) kept uncapped; volatile tail capped at `pc_rel_volatile_tail_cap` (default 10) after mention-force-keep; 109-rel PC web shrinks to ~30–40. L4 adds the same type-tiering to `_collect_existing_relationships` so permanent bonds survive the token-budget degradation step. Gated under `context_optimizations.relationship_type_tiering` in `config/llm.json` (default OFF). A/B validation (344t, same-commit flag-OFF control, port-8000 lane, P1) is a separate follow-up task.
